  ...
  return (
    <div className="feature-page settings-page">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card ai-settings-card"><div className="settings-card-title"><span><Icon name="sparkles" /></span><div><h2>AI configuration</h2><p>Server-side connection status</p></div></div>{settings ? <div className="ai-status"><span className={settings.ai.configured ? "connected" : "local"}>{settings.ai.configured ? "OpenAI connected" : "Limited local mode"}</span><strong>{settings.ai.configured ? settings.ai.model : "No OpenAI API key configured"}</strong><p>{settings.ai.configured ? "The assistant can answer free-form questions and remember the recent conversation. Your key stays on the server and is never sent to the browser." : "The built-in fallback only recognises a few job-search categories. Connect OpenAI to ask free-form questions and use natural follow-ups."}</p>{!settings.ai.configured && <div className="ai-setup"><h3>Connect OpenAI</h3><ol><li>Create an API key in the OpenAI dashboard.</li><li>Open the <code>.env</code> file in the project root.</li><li>Set <code>OPENAI_API_KEY=your-key-here</code>.</li><li>Save the file, then restart Docker with <code>docker compose down</code> followed by <code>docker compose up --build</code>.</li></ol><a className="button secondary" href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer">Create OpenAI API key</a><small>Never paste your key into the app, GitHub, or chat. Keep <code>.env</code> private.</small></div>}</div> : <div className="settings-loading"><span className="spinner" /></div>}</section>
